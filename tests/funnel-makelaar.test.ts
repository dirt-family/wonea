...
describe("createLead via funnel:verkopen (makelaar)", () => {
  it("maakt lead + consent + event + bevestigingsmail met de juiste leadwaarde", async () => {
    const { leadId } = await leadsLib.createLead({
      type: "makelaar",
      adresId,
    ...
    const mail = (await db.select().from(schema.emailsOutbox)).find((m) => m.to === "verkoper@voorbeeld.nl");
...
describe("createLead via funnel:taxatierapport (taxatie)", () => {
  it("maakt lead + consent + event + bevestigingsmail met de juiste leadwaarde", async () => {
    ...
    const mail = (await db.select().from(schema.emailsOutbox)).find((m) => m.to === "taxatie@voorbeeld.nl");
    expect(mail).toBeDefined();
    expect(mail!.type).toBe("lead_bevestiging");
...
describe("suppressie", () => {
  it("weigert een lead voor een gesuppresseerd adres en laat geen sporen achter", async () => {
    const suppressedId = (
      await db
        .insert(schema.addresses)
        .values({
    ...
    const leadRijen = await db.select().from(schema.leads).where(eq(schema.leads.email, "geweigerd@voorbeeld.nl"));
    expect(leadRijen).toHaveLength(0);
    const consentRijen = await db.select().from(schema.consents).where(eq(schema.consents.email, "geweigerd@voorbeeld.nl"));
