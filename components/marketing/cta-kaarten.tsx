...
export function CtaKaarten() {
  ...
  return (
    <div className="divide-y divide-lijn overflow-hidden rounded-[14px] border border-lijn bg-paneel">
      {ctas.map((cta) => (
        <Link key={cta.href} href={cta.href} className="group flex items-center gap-5 p-6 transition-colors hover:bg-merk-50">
          ...
          <span
            aria-hidden="true"
            className="text-2xl text-merk-300 transition-transform duration-200 group-hover:translate-x-1 group-hover:text-merk"
          >
            &#8594;
          </span>
        </Link>
